...
function reshapeDummy(parentw, parenth) {
	return {
		left: 0,
		top: 0,
		width: 2,
...
function updateRoomStatus (roomName, otherPeople) {
	var count = 1;
	var status;
	...
	if (count < 4) {
		status = 'Waiting for '+(4-count)+' participant(s)';
	} else {
		status = 'All participants have joined';
	}
	document.getElementById('conf_status').innerText = status;
}
function callEverybodyElse(roomName, otherPeople) {

	updateRoomStatus(roomName, otherPeople);
	easyrtc.setRoomOccupantListener(updateRoomStatus); // so we're only called once.
	...
	var candidate = null;
	for (var easyrtcid in otherPeople) {
		console.log("[Other] "+easyrtcid+": "+easyrtc.idToName(easyrtcid));
		if (easyrtc.idToName(easyrtcid) == "candidate") {
			// leave candidate to the last one
			candidate = easyrtcid;
			console.log("Found candidate: "+easyrtcid);
			continue;
		} else {
			list.push(easyrtcid);
		}
...
function appInit(roomName,userName,isEmployerConf) {

	// Prep for the top-down layout manager
	//setReshaper('fullpage', reshapeFull);
	setReshaper('fullpage', reshapeDummy);
	...
	setReshaper('textentryBox', reshapeDummy);
	setReshaper('textentryField', reshapeDummy);
	setReshaper('textEntryButton', reshapeDummy);
...
var allMuted = false;
var allBlind = false;
function toggleBlindAll () {
	allBlind = !allBlind;
	var className;
	var visibility;
	if (allBlind) {
		className = 'cam_icon cam_video_blind';
		visibility = 'hidden';
	} else {
		className = 'cam_icon cam_video';
		visibility = 'visible';
	}
	document.getElementById('video_button').setAttribute('class', className);
	document.getElementById('box1').style.visibility = visibility;
	document.getElementById('box2').style.visibility = visibility;
	document.getElementById('box3').style.visibility = visibility;
}

function toggleMuteAll () {
	allMuted = !allMuted;
	if (allMuted) {
		document.getElementById('audio_button').setAttribute('class', 'cam_icon cam_audio_muted');
	} else {
		document.getElementById('audio_button').setAttribute('class', 'cam_icon cam_audio');
	}
	easyrtc.muteVideoObject('box1', allMuted);
	easyrtc.muteVideoObject('box2', allMuted);
	easyrtc.muteVideoObject('box3', allMuted);
}
